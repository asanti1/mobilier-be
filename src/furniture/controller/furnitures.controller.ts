...
import { Furniture } from '../../furniture/interface/furniture.intefaces';
...
router.post(
  ...
  [
    ...
    extractToken, verifyToken, fieldsValidator,],
...
router.put('/:id', [check('id', 'it is not a valid id').isMongoId(),
  extractToken, verifyToken, fieldsValidator], async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
